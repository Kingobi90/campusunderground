...
# Mock data provider removed - using real Moodle data only
...
class MoodleAPIClient:
    ...
    def __init__(self):
        ...
        Requires valid MOODLE_URL and MOODLE_API_TOKEN environment variables.
        """
        self.base_url = os.getenv('MOODLE_URL', 'https://moodle.concordia.ca')
        self.token = os.getenv('MOODLE_API_TOKEN', '')
        
        # Validate required environment variables
        if not self.token or self.token == 'your_moodle_api_token_here':
            raise ValueError("MOODLE_API_TOKEN environment variable is required and must be set to a valid token")
        ...
        logger.info("Using real Moodle API data only")
    ...
    def _call_api(self, wsfunction: str, **params) -> Dict[str, Any]:
        ...
        """
        # Real Moodle API calls only - no mock data fallback
    ...
    def validate_token(self) -> bool:
        ...
        """
        # Real token validation only - no mock data fallback
    ...
    def get_courses(self, user_id: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        for course in courses:
            ...
            if 'progress' not in course:
                course['progress'] = 0  # Progress calculation would require additional API calls
    ...
    def download_file(self, file_url: str) -> Dict[str, Any]:
        ...
        """
        # Real Moodle API calls only - no mock data fallback
        if False:  # Disabled mock data block
